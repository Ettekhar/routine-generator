...
export async function generateFullSchedule() {
  ...
  for (const course of courses) {
    ...
    for (const bc of course.batches) {
      ...
      // ------ LAB COURSE ------
      // ---------- LAB COURSE HANDLING ----------
      const isLab = course.type === CourseType.LAB && bc.requiresLab === true;

      if (!isLab) {
        // THEORY or non-lab → NO groups
        assigns.push({
          courseId: course.id,
          teacherId: teacherInfo.id,
          batchId: bc.batchId,
      ...
      // ---------- LAB: Fetch groups from DB ----------
      const labGroups = await prisma.labGroup.findMany({
        where: { batchCourseId: bc.id }, // ✅ correct field
      });
      console.log({labGroups})
      let groupsForSchedule: { id: number; name: string; size: number }[] = [];

      // CASE 1: LAB GROUPS EXIST IN DB → use them
      if (labGroups.length > 0) {
        for (const g of labGroups) {
          // Find/ensure BatchGroup matches LabGroup
          let bg = await prisma.batchGroup.findFirst({
            where: {
              batchId: bc.batchId,
              name: `${batchName}-${g.name}`,
            },
          });

          if (!bg) {
            bg = await prisma.batchGroup.create({
              data: {
                batchId: bc.batchId,
                name: `${batchName}-${g.name}`,
                size: g.size,
      ...
      // CASE 2: NO LAB GROUPS → AUTO-SPLIT INTO A & B
      else {
        console.log(`Auto-splitting ${batchName} into Group A & B for ${course.title}`);

        const splitNames = ["Group A", "Group B"];
        const eachSize = Math.ceil(batchSize / 2);

        for (const name of splitNames) {
          // Create BatchGroup if missing
          let bg = await prisma.batchGroup.findFirst({
            where: {
              batchId: bc.batchId,
              name: `${batchName}-${name}`,
