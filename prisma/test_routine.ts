...
// NEW SOFT PENALTIES (Implemented)
const PENALTY_CONSECUTIVE_SAME_COURSE = -200;  // Same subject back-to-back
const PENALTY_LAB_MISALIGN = -3000;            // Group A/B labs not same day
const PENALTY_OVERLOADED_DAY = -500;           // More than 3 classes in one day
const PENALTY_TEACHER_NO_BREAK = -1500;        // Teacher teaches 3+ hrs without break
...
// Slot information for schedule tracking
  ...
  for (const other of chromosome) {
    ...
    if (isTimeOverlap(gene, other)) {
      ...
      if (other.batchId === gene.batchId) {
        // Conflict only if they belong to the same group or both are full-batch (groupId: null)
        // or one is full-batch and the other is a group.
        if (
          other.groupId === null ||
          gene.groupId === null ||
...
function repairChromosome(
  ...
  for (const gene of chrom) {
    ...
    const room = lookups.roomMap.get(gene.roomId);
    // Check for missing room first to avoid error
    if (!room) {
        gene.invalid = true;
        continue;
    }
...
function calculateFitness(chrom: Chromosome, lookups: GALookupData): number {
  ...
  for (const gene of chrom) {
    ...
    // Check for missing room (should have been caught in repair, but double check)
    if (!room) {
        fitness += HARD_PENALTY;
        continue;
    }
    ...
    for (let h = 0; h < gene.hours; h++) {
      ...
      const groupKey = gene.groupId ? `G-${gene.batchId}-${gene.groupId}-${gene.day}-${t}` : `B-${gene.batchId}-${gene.day}-${t}`;
      ...
      if (used.has(groupKey) && gene.courseType !== CourseType.THEORY) { // Groups/Batches should not clash
          // Note: Full batch (Theory) is handled by B-batchId key. Grouped Labs are G-batchId-groupId key.
          // This allows different lab groups of the same batch to run concurrently.
          // But if one is Theory (B-key) and the other is a Lab (G-key), they won't conflict. 
          // We need a shared batch key for all batch activities.
          const batchConflictKey = `B-ALL-${gene.batchId}-${gene.day}-${t}`;
          if (used.has(batchConflictKey)) {
              // This is a batch conflict where the whole batch is busy with something else
              fitness += ROOM_CONFLICT_PENALTY; // Using the same weight for severity
              conflict = true;
          }
          used.add(batchConflictKey);
      }
      
      used.add(teacherKey);
      used.add(roomKey);
      used.add(groupKey); // For group-level conflict tracking
    }
    ...
    if (start < 4) fitness += BONUS_MORNING; // 09:00, 10:00, 11:00, 12:00
    if (start >= 6) fitness += PENALTY_LATE_CLASS; // 03:00, 04:00

    assigned.set(key, (assigned.get(key) ?? 0) + gene.hours);

    // Build batch schedule (including courseId for consecutive course check)
    ...
    // Build teacher schedule (including hours for break check)
  ...
  // ============================
  // PASS 2: Batch Day Analysis (Gaps, Isolation, Consecutive Courses, Overload)
  // ============================
  for (const [batchId, dayMap] of batchSchedules) {
    
    // Penalize active days (encourages compact schedule over fewer days)
    ...
    for (const [day, slots] of dayMap) {
      ...
      for (let i = 0; i < slots.length; i++) {
          ...
          // PENALTY_CONSECUTIVE_SAME_COURSE (Soft constraint)
          if (prev && current.start === prev.end && current.courseId === prev.courseId) {
              fitness += PENALTY_CONSECUTIVE_SAME_COURSE;
          }
          
          // Track consecutive teaching for overload/break analysis (re-used here)
          if (prev && current.start === prev.end) {
              if (consecutiveTeaching === 0) {
                  consecutiveTeaching = prev.hours + current.hours;
              } else {
                  consecutiveTeaching += current.hours;
              }
          } else {
      ...
      // PENALTY_OVERLOADED_DAY (More than 3 classes (genes) in one day)
      // Note: A 2-hour lab counts as one gene for scheduling purposes, but has 2 hours.
      // We check number of slots (genes)
      if (slots.length > 3) { 
          fitness += PENALTY_OVERLOADED_DAY * (slots.length - 3);
      }
    }
  }

  // ============================
  // PASS 3: Teacher Workload (Daily Load, No Break)
  // ============================
  for (const [teacherId, dayMap] of teacherSchedules) {
    ...
    for (const [day, slots] of dayMap) {
      ...
      // PENALTY_TEACHER_NO_BREAK (Soft constraint)
      ...
      for(let i = 0; i < slots.length; i++) {
          ...
          if (prev && current.start === prev.end) {
              // Consecutive
              if (currentConsecutiveHours === 0) {
                  currentConsecutiveHours = prev.hours + current.hours;
              } else {
          ...
          // Penalize if teaching 3 or more straight hours
          if (currentConsecutiveHours >= 3) {
              fitness += PENALTY_TEACHER_NO_BREAK * (currentConsecutiveHours - 2); 
          }
      }
    }
  }
  ...
  // ============================
  // PASS 4: Missing Hours (Hard Constraint)
  // ============================
  ...
  // ============================
  // PASS 5: Lab Group Alignment (Soft Constraint)
  // ============================
  // Group labs by course and batch (A and B groups should be on the same day)
  const labGroupMap = new Map<string, PlacedAssignment[]>(); // Key: C-B

  for (const gene of chrom) {
      // Only interested in scheduled lab groups (not full batch theory)
  ...
  for (const [key, labs] of labGroupMap) {
      // Check how many unique days the *groups* for this course/batch appear on.
      const groupDayMap = new Map<number, Set<Day>>(); // groupId -> Set<Day>
      ...
      // We only care if there are multiple sibling groups (e.g., Group 1 and Group 2)
      if (groupDayMap.size > 1) { 
          const allDays = new Set<Day>();
          
          // Collect all unique days used by all groups for this course/batch
          groupDayMap.forEach(days => days.forEach(day => allDays.add(day)));

          // If there is more than one unique day, it means the groups are scheduled
          // on different days (misaligned).
          if (allDays.size > 1) {
              // Apply penalty proportional to misalignment (e.g., 3 unique days -> penalty * 2)
              fitness += PENALTY_LAB_MISALIGN * (allDays.size - 1);
          }
      }
  }
...
function crossover(a: Chromosome, b: Chromosome): [Chromosome, Chromosome] {
  ...
  for (let i = 0; i < max; i++) {
    const g1 = a[i];
    const g2 = b[i];
    // Simple 50/50 chance swap, preserving gene count integrity isn't necessary here
    // as the GA handles missing genes with MISSING_HOURS_PENALTY
    if (Math.random() < 0.5) {
...
async function runGA(
  ...
  for (let gen = 0; gen < MAX_GENERATIONS; gen++) {
    ...
    population = next;
    // Recalculate best after sorting
...
// --------------------------------------------------
// HELPER: Helper for Time Minutes (Removed as it's not used)
